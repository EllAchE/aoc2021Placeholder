...
def magnitude(flat):
    for lvl in range(0, 5):
        for i, f in enumerate(flat):
            if i + 1 < len(flat):
                if f[1] == 4-lvl and flat[i+1][1] == f[1]:
...
max = 0
for add1 in parsedLst: # order of addition matters
    for add2 in parsedLst:
        if add1 != add2:
            print add1, add2
            addTree = parseTree(add1, add2)
            flat = flatTree([], addTree, 0)  # kind of wasteful to parse it in and out of a tree but whatever
